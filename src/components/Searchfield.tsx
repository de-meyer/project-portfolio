...
export default function Searchfield({searchForUser}: {searchForUser: (username:string)=> void}) {
   ...
   const searchOnEnter = (event: { key: string; }) => {
    ...
    return (<>
    <div className="">
        <input className="text-black text-s inline-flex select-none rounded-lg px-6 py-3 text-center align-middle font-sans font-bold " type="text" onKeyDown={searchOnEnter} placeholder={githubUsername}  onChange={(event)=> setGithubUsername(event.target.value)}/>
